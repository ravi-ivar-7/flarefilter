                                ...
                                <div className="flex justify-between items-center gap-4">
                                    ...
                                    <div className="flex items-center gap-2 shrink-0">
                                        ...
                                        <Form method="post" onSubmit={(e: React.FormEvent) => {
                                            ...
                                            <input type="hidden" name="zoneId" value={zone.id} />
                                            <button type="submit" className="p-1.5 text-rose-500 hover:text-rose-700 hover:bg-rose-50 rounded-md transition-colors">
                                                ...
                                                {zoneRules.map((rule) => (
                                                    <div key={rule.id} className={`flex items-center gap-4 px-5 py-3 bg-white border ${rule.isActive ? 'border-slate-200' : 'border-slate-100 opacity-60'} shadow-sm rounded-md group/rule hover:border-indigo-400 transition-all`}>
                                                        <Form method="post" className="flex items-center flex-shrink-0">
                                                            ...
                                                            <input type="hidden" name="ruleType" value={rule.type} />
                                                            <button
                                                                type="submit"
                                                                disabled={!zone.isActive}
...
function RuleDetails({ rule }: { rule: any }) {
    ...
    if (config && config.renderDetails) {
        return <>{config.renderDetails(rule, config)}</>;
    }

    return (
        <div className="flex items-center gap-3 flex-1 min-w-0 flex-wrap sm:flex-nowrap">
            <span className="text-xs font-black text-slate-900 uppercase tracking-tight shrink-0">Unknown Rule: {rule.type}</span>
        </div>
    );
}
