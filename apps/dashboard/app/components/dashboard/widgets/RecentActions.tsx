        ...
        <div className="bg-white border border-gray-200 rounded-md shadow-sm overflow-hidden flex flex-col h-full w-full">
            ...
            <div className="p-5 space-y-4 flex-1 overflow-y-auto min-h-0 custom-scrollbar">
                {actions.length === 0 ? (
                    ...
                    actions.map((action) => (
                        <div key={action.id} className="p-4 rounded-md border border-rose-100 bg-gradient-to-br from-white to-rose-50/30">
                            <div className="flex justify-between items-center mb-2">
                                <span className="font-mono text-xs text-rose-600 bg-rose-50 border border-rose-100 px-2 py-0.5 rounded-md font-black shadow-sm tracking-tight">{action.targetValue}</span>
                            ...
                            <div className="bg-white/60 p-2.5 rounded-md border border-slate-100 shadow-sm">
                                <span className="text-xs text-slate-700 font-medium tracking-tight">
                                    Targeted <span className="text-rose-600 font-black">{action.requestCount?.toLocaleString()}</span> malicious requests
                                </span>
                            </div>
                        </div>
                    ))
